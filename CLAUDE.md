...
- **NFL data ingest pipeline** — three layered services, each authoritative for one slice of state. Run via `/nfl-rebuild` (full) or `/nfl-refresh` (weekly). Layers:
  1. **`Nflverse::SeedPlayers`** (`app/services/nflverse/seed_players.rb`, rake `nfl:players_seed`) — identity backbone. Pulls `players.csv` from nflverse-data GitHub release (~24k rows, filter to `status=ACT AND last_season>=2024`), upserts Person + Athlete with all five cross-ref IDs (`gsis_id`, `espn_id`, `pff_id`, `otc_id`, `pfr_id`, `nflverse_id`). Lookup priority is `gsis_id` (anchor) → `person_slug` (name match) → create. Sets `Athlete.team_slug` from `latest_team` and caches ESPN headshots to S3 inline (idempotent, skipped without AWS creds). The `team_slug` here is provisional — Spotrac and ESPN authoritatively overwrite below.
  ...
  3. **`Espn::ScrapeDepthCharts`** (`app/services/espn/scrape_depth_charts.rb`, rake `espn:scrape_depth_charts`) — current-roster + depth truth. Hits `https://www.espn.com/nfl/team/depth/_/name/{abbrev}` per team (data embedded in `window['__espnfitt__']`). Auto-creates DepthChart shells and Contracts for ESPN-listed players we don't have yet (UDFAs, mid-season call-ups). When a player has shifted teams, expires the old active Contract and creates the new one. Updates `Athlete.team_slug`. Locked DepthChartEntry rows are never moved. Position labels normalized via `PositionConcern.normalize_position(raw, source: :espn)` — collapses formation-specific WLB/LILB/LDE/FS to canonical LB/EDGE/S.
...
- **Depth chart pipeline** — `Roster#pick_starters` and `Roster#special_teams_starting_4` read `DepthChartEntry` rows at runtime. Two ways entries land:
  - Live: `bin/rails espn:scrape_depth_charts` (preferred — current week's depth per ESPN).
  - Fallback: `db/seeds/31_depth_charts.rb` ranks by `overall_grade DESC, salary DESC` from active contracts. Used during plain `db:seed` when ESPN isn't run.
  - UI edits via `/teams/:slug/depth-chart` (drag-reorder, lock toggle) flow immediately to `/nfl-rosters`. Locked entries are skipped by both the seed re-rank AND the ESPN scrape — manual overrides survive both refreshes.

- **Position normalization** — `PositionConcern` (`app/models/concerns/position_concern.rb`) holds canonical position lists plus per-source mapping tables (`ESPN_MAP`, `PFF_MAP`, `NFLVERSE_MAP`, `SPOTRAC_MAP`, `GENERAL_MAP`). Callers pass `source:` to dispatch: `PositionConcern.normalize_position("LDE", source: :espn) # => "EDGE"`. Falls back to `GENERAL_MAP` when source is omitted or has no entry for the input.

- **Athlete cross-ref IDs** — `Athlete` has columns for every external system's player ID: `gsis_id` (NFL canonical), `espn_id`, `pff_id`, `otc_id` (Spotrac/OverTheCap), `pfr_id` (Pro-Football-Reference), `nflverse_id`. nflverse's master CSV provides them all in one row. Importers prefer ID-based matching (exact, durable) over name matching (fragile across "Patrick Mahomes II" vs "Patrick Mahomes"). All five have unique indexes.
...
| | `32_headshot_links.rb` | DB-only. Sets `espn_id` + `espn_headshot_url` on Athletes (from nflverse roster CSV) and on Coaches (HCs from ESPN, all 4 roles scraped from each team's NFL.com `/team/coaches/` page via `TEAM_NFL_SUBDOMAIN` map in `lib/tasks/nfl.rake`). No S3 traffic. Largely superseded by `nfl:players_seed` for athletes; coach links still authoritative here. |
