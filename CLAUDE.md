...
- **DepthChart** — team_slug (unique), slug. `include Sluggable`, `name_slug` = `"#{team_slug}-depth"`. One per NFL team, has many DepthChartEntry. Canonical source for "who plays where + depth ordering" — drives `Roster#pick_starters` and `/nfl-rosters`. Manageable via `/teams/:slug/depth-chart` UI.
- **DepthChartEntry** — depth_chart_slug (FK), person_slug (FK to Person), position, side (offense/defense/special_teams), depth (1..N within position), locked (bool). Unique on `[depth_chart_slug, person_slug, position]`. Locked rows are skipped by the seed re-rank AND by `Espn::ScrapeDepthCharts` — manual overrides survive both refreshes.
...
- **PFF grade pipeline** — `db/seeds/data/pff/*.csv` (committed) → `db/seeds/29_pff_grades.rb` → `Pff::ImportCsv` (`app/services/pff/import_csv.rb`). Detects stat_type from filename. Each stat_type has a `POSITION_FILTER` whitelist so only relevant positions get grades written (e.g. `passing_summary` → QB only, `offense_pass_blocking` → OL+TE). For blocking CSVs without `grades_offense`, derives `overall_grade` as the avg of `pass_block_grade` + `run_block_grade`; for defensive sub-CSVs without `grades_defense`, derives from coverage/pass_rush/rush_defense. Synthetic salary heuristic in `30_athlete_grades.rb` only fills the gap for non-PFF athletes (flat 60.0 baseline). Drop new CSVs into the dir and re-seed; idempotent via `find_or_initialize_by`. Currently imported: passing/rushing/receiving + offense_pass_blocking + offense_run_blockng + pass_rush + run_defense + defense_coverage + defense_summary + field_goal + return = 11 stat types.
- **Depth chart pipeline** — `db/seeds/31_depth_charts.rb` (runs before `31_rosters.rb`) builds one `DepthChart` per NFL team and one `DepthChartEntry` per `(person, position)` for every active contract. Unlocked entries rank by `overall_grade DESC, salary DESC`; locked entries hold their `(depth, position)` across re-seeds. `Roster#pick_starters` and `Roster#special_teams_starting_4` read entries from DepthChart at runtime — UI edits via `/teams/:slug/depth-chart` flow through immediately to `/nfl-rosters`. `RosterSpot.depth` is denormalized from DepthChart at seed time; pick_starters does NOT consult it. Tests sync DepthChart from RosterSpot fixtures via the `setup` block in `RosterTest`.
- **ESPN depth chart scraper** — `Espn::ScrapeDepthCharts` (`app/services/espn/scrape_depth_charts.rb`), called via `bin/rails espn:scrape_depth_charts [TEAM=buf] [VERBOSE=1]`. Hits `https://www.espn.com/nfl/team/depth/_/name/{abbrev}` for each of 32 teams; depth chart data is embedded as JSON in `window['__espnfitt__']` (`page.content.depth.dethTeamGroups`). Behavior: collapses ESPN's formation labels (LDE/RDE→EDGE, WLB/SLB/LILB/RILB→LB, FS/SS→S, PK→K) to dominant contract forms; flattens multi-row positions round-robin (WR1+WR2+WR3 starters slot into depths 1-3); skips players whose active contract is on a different team (ESPN drift); skips `H/KR/PR` rows (we model returners by `return_grade` sort, not as a depth slot); cross-position dedup ensures one entry per player (moves entry to ESPN's position rather than duplicating); densifies depths post-scrape. Locked entries are never moved.
...
| 4. Evaluation | `29_pff_grades.rb` | Imports `db/seeds/data/pff/*.csv` via `Pff::ImportCsv`. Currently 11 stat types: passing/rushing/receiving/offense_pass_blocking/offense_run_blockng/pass_rush/run_defense/defense_coverage/defense_summary/field_goal/return. Position-filtered per stat type. ~570 graded athletes when all CSVs present. |
| | `30_athlete_grades.rb` | Synthetic fallback. Non-PFF NFL athletes get a flat 60.0 baseline; prospects keep tier-based ranges (JSONB `grade_ranges`) since draft slot is the only signal. |
| | `31_depth_charts.rb` | One DepthChart per NFL team, one DepthChartEntry per (person, position). Unlocked rank by `overall_grade DESC, salary DESC`; locked entries hold position+depth across re-seeds. |
| | `31_rosters.rb` | 64 rosters, ~5000 roster spots. Depth denormalized from DepthChart. |
| | `32_headshot_links.rb` | DB-only. Sets `espn_id` + `espn_headshot_url` on Athletes (from nflverse roster CSV) and on Coaches (HCs from ESPN, all 4 roles scraped from each team's NFL.com `/team/coaches/` page via `TEAM_NFL_SUBDOMAIN` map in `lib/tasks/nfl.rake`). No S3 traffic. ~21 coach name mismatches are normal (seed vs current rosters) and logged as `[~]`. |
| | `33_headshot_uploads.rb` | Reads `db/seeds/data/headshots/nfl/` (committed via Git LFS, 1,679 originals / 414MB) and uploads each to S3 + generates 100w/400w variants via `Studio::ImageCache.cache!(source_path:)`. Creates one `ImageCache` row per variant. Idempotent — re-seed finishes in seconds. **Offline-capable** — no ESPN/NFL.com calls during `db:seed`. Skipped when `AWS_ACCESS_KEY_ID` blank or local files absent (run `git lfs pull` first). Coverage: 1,570 athletes + 109 coaches × 3 variants = 5,037 rows. |
